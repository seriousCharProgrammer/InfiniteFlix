...
export default function Recommends() {
  const dispatch = useDispatch();
  const { popularMovies, loading, totalPages } = useSelector(
    (state) => state.movies
  );
  const navigate = useNavigate();

  // State for tracking hover movie id and pagination
  const [hoveredMovie, setHoveredMovie] = useState(null);
  ...
  // Track when fetching is complete
  useEffect(() => {
    if (!loading) {
      setIsFetching(false);
    }
  ...
  return (
    <Container>
      ...
      {page >= totalPages && (
        <NoMoreResultsMessage>No more movies to load</NoMoreResultsMessage>
      )}
    </Container>
  );
}
...
const NoMoreResultsMessage = styled.div`
  color: #f9f9f9;
  text-align: center;
  margin-top: 20px;
  font-size: 16px;
